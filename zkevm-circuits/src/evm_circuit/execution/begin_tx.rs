use crate::{
    evm_circuit::{
        ...
        util::{
            ...
            is_precompiled,
            math_gadget::{
                ContractCreateGadget, IsEqualWordGadget, IsZeroWordGadget, MulWordByU64Gadget,
                RangeCheckGadget,
            },
    ...
    fn configure(cb: &mut EVMConstraintBuilder<F>) -> Self {
        ...
        cb.condition(tx_is_create.expr(), |cb| {
            cb.require_equal(
                ...
                expr_from_bytes(&caller_nonce_hash_bytes.limbs[0..N_BYTES_ACCOUNT_ADDRESS]),
            );
        });
        cb.require_equal(
    ...
    fn assign_exec_step(
        ...
        self.tx_caller_address.assign(
            ...
            tx.caller_address
                .to_fixed_bytes()
                .iter()
                .rev() // From H160 big endian to small endian
                .map(|b| *b)
                .collect_vec()
                .try_into()
                .ok(),
        ...
        self.tx_callee_address.assign(
            ...
            tx.callee_address
                .to_fixed_bytes()
                .iter()
                .rev() // From H160 big endian to little endian
                .map(|b| *b)
                .collect_vec()
                .try_into()
                .ok(),
        ...
        self.is_empty_code_hash
            .assign_u256(region, offset, callee_code_hash, zero)?;
        ...
        self.caller_nonce_hash_bytes
            .assign(region, offset, Some(untrimmed_contract_addr))?;
        self.create.assign(
            region,
            offset,
            tx.caller_address,
