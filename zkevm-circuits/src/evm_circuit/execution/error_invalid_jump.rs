...
impl<F: Field> ExecutionGadget<F> for ErrorInvalidJumpGadget<F> {
    ...
    fn assign_exec_step(
        ...
        let dest = usize::try_from(dest).unwrap_or(code.code.len());
        let code_pair = code
            .get(dest)
            .map_or((0u64, false), |elem| (elem.value.into(), elem.is_code));

        self.value
            .assign(region, offset, Value::known(F::from(code_pair.0)))?;
        self.is_code
